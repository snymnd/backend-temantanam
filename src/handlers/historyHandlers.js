...
const getHistoryHandler = async (request, h) => {
    ...
    let query = `SELECT h.id AS history_id, h.created_at AS created_at, p.id AS plant_id, p.name AS plant_name, p.species AS plant_species, p.image_url AS plant_image_url, p.classification AS plant_classification
                FROM history h
